...
1. 高难/典故挑战模式尚未做显式入口；当前策略是保留高难题数据，但普通入口完全不抽。
2. 双人模式仍需要更完整的异常流：退出、重连、房间过期、重复提交幂等。
3. 首页和提示猜词页面已经统一视觉，历史页、个人页、双人页后续也应逐步统一到“成语日课”设计语言。
4. 已关闭：双人分享深链未消费 `options.code`，当前分享房间冷启动会自动切到加入页并预填房间码。
5. 已关闭：精选默认题池少于 200 条，本轮扩到 229 条大众题。
6. 已关闭：按钮文案 emoji 质感问题，主流程按钮已去 emoji；结果表情、分享战绩符号和底部导航图标按产品口径保留。
7. 已关闭：提示猜词未结算分享弱文案，当前默认分享为邀请型文案，不再出现 `0/5 · 0分`。
8. 已关闭：提示猜词和相关“返回首页”按钮无反馈，已改为稳定 `reLaunch('/pages/home/home')` 并通过 MCP 回归。
...
- MCP `reLaunch` 到 `pages/idiom-match/idiom-match` 后，抽样得到 `囫囵吞枣 Lv.1`、`井底之蛙 Lv.1`、`车水马龙 Lv.1`、`女娲补天 Lv.1`、`四面楚歌 Lv.2`、`红豆相思 Lv.2`，未出现 Lv.3/Lv.4。
